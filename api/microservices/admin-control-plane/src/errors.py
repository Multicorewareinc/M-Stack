"""Application errors and their handlers. Reuses the model-gateway/rate-limiter JSON
envelope `{"error": {"message": ..., "type": ...}}` (ADR-003). Unlike the thin proxies,
this service raises typed 4xx errors from its CRUD services (404/409/422) and an auth 401,
each rendered by the same handler."""
...
class UpstreamError(AppError):
    # A dependency (e.g. Org CP's internal API) was unreachable or returned an unexpected status.
    # The org-creation workflow persists a `failed` org and surfaces this rather than a 500.
    status_code = 502
    error_type = "upstream_error"


# ── Local auth errors (modules/auth, design D6) ──────────────────────────────────────────────
...
# This is the SP-01 auth error set MINUS org_required (super-admins are org-less).
...
class TooManyAttemptsError(AppError):
    ...
    def __init__(self, *, retry_after: int) -> None:
        super().__init__()
        # Remaining lock TTL, surfaced as the Retry-After header (read by install_error_handlers
        # below via getattr).
