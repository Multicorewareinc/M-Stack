"""NATS JetStream consumer — the async counting strain (ADR-013).

Subscribes to the gateway's event backbone (ADR-007/008) and, for each `request` event,
increments the RPM counters via the service. This is what makes RPM counting happen off the
hot path: the gateway publishes one `request` event per forwarded call, and this consumer
turns that into the Redis INCRs that `/check` later reads.
...
class RpmConsumer:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.warning("rpm_consumer_connect_failed_retrying", exc_info=True)
            self._nc = None
    ...
    async def _bind(self) -> None:
        ...
        try:  # the gateway also creates this; add_stream is idempotent
            ...
            logger.info("rpm_stream_exists", extra={"stream": self._stream})
        try:
            ...
            logger.info("rpm_consumer_started", extra={"subject": self._subject, "durable": self._durable})
        except Exception as exc:
            self._sub = None
            text = str(exc).lower()
            if "already" in text and ("exist" in text or "bound" in text or "in use" in text):
                logger.warning("rpm_consumer_durable_conflict", exc_info=True)  # another replica holds it
            else:
                logger.warning("rpm_consumer_bind_failed", exc_info=True)  # transient — will retry on reconnect
    ...
    async def _on_reconnected(self) -> None:
        ...
        if self._sub is not None:
            try:
                ...
                logger.warning("rpm_resubscribe_drain_failed", exc_info=True)
            self._sub = None
        await self._bind()
    ...
    async def _on_disconnected(self) -> None:
        ...
        self._sub = None
        logger.warning("rpm_consumer_disconnected")
        ...
        try:
            ...
            if isinstance(event, dict) and event.get("type") == "request":
                await self._service.count(
                    event.get("principal"), event.get("model"), event.get("path"), event.get("request_id")
                )
        except Exception:
            logger.warning("rpm_consume_error", exc_info=True)
        finally:
            try:
                await msg.ack()  # soft: ack even on error so a poison msg can't loop forever
            except Exception:
                logger.warning("rpm_ack_failed", exc_info=True)
    ...
    async def stop(self) -> None:
        if self._nc is not None:
            try:
                ...
                logger.warning("rpm_consumer_stop_failed", exc_info=True)
