...
the service. NOT behind require_service_key / require_org_context (this is end-user auth, D6/8.3).
...
def _login_body(outcome: service.LoginOutcome) -> dict:
    ...
    return LoginResponse(
        ...
        org=OrgOut.from_org(issued.org) if issued.org is not None else None,
...
async def me(request: Request, user=Depends(get_current_user)) -> Response:
    async with request.app.state.sessionmaker() as session:
        org = await service.resolve_user_org(session, user)
        permissions = await service._resolve_permissions(session, org, user)
    body = MeResponse(
        user=UserOut.from_user(user),
        org=OrgOut.from_org(org) if org is not None else None,
