...
from conftest import JWT_SECRET, asgi_client, init_schema, make_auth_app, seed_user
...
async def test_no_secret_material_in_logs(fake_redis, caplog):
    ...
    uid, _org, _pw = await seed_user(sm, email="log@acme.com", username="log", password=PW)
    new_pw = "R0tatedLogPass!!"

    async with asgi_client(app) as c:
        # success login → capture the refresh token issued
        c.cookies.clear()
        r = await c.post("/api/auth/login", json={"email": "log@acme.com", "password": PW})
        access = r.json()["access_token"]
        raw_refresh = r.cookies["refresh"]
        # a failed login (wrong password)
        c.cookies.clear()
        await c.post("/api/auth/login", json={"email": "log@acme.com", "password": "wrong-but-long"})
        # refresh + logout
        c.cookies.clear()
