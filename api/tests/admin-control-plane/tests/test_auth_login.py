"""Endpoint: POST /api/auth/login — success + cookie flags + role super_admin + NO org claim +
last_login_at, no account enumeration, deactivation-after-verify, local-auth-disabled short-circuit,
DB-error fails closed (500 not 401), over-length password rejected pre-hash. Maps spec + task 9.4."""
...
async def test_login_success_cookie_role_no_org_last_login(fake_redis):
    app = await _app()
    sm = app.state.sessionmaker
    uid, pw = await seed_admin(sm, email="alice@platform.local", password=PW)
    async with asgi_client(app) as c:
        r = await c.post("/api/auth/login", json={"email": "alice@platform.local", "password": pw})
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["token_type"] == "Bearer"
    assert body["expires_in"] == 3600
    assert body["user"]["email"] == "alice@platform.local"
    assert body["user"]["role"] == "super_admin"
    assert "org" not in body and "permissions" not in body  # org-less super-admin
    assert body["must_change_password"] is False
    # Access token carries sub/email/role and NO org claim (decoded with the configured secret).
    claims = auth_jwt.decode_access_token(body["access_token"], secret=SecretStr(JWT_SECRET))
    assert claims["sub"] == str(uid) and claims["role"] == "super_admin"
...
async def test_no_enumeration_identical_401(fake_redis, monkeypatch):
    ...
    app = await _app()
    sm = app.state.sessionmaker
    await seed_admin(sm, email="real@platform.local", password=PW)
    await seed_admin(sm, email="nohash@platform.local", with_hash=False)
    ...
    async with asgi_client(app) as c:
        r_unknown = await c.post("/api/auth/login", json={"email": "ghost@platform.local", "password": PW})
        r_nohash = await c.post("/api/auth/login", json={"email": "nohash@platform.local", "password": PW})
        r_wrong = await c.post("/api/auth/login", json={"email": "real@platform.local", "password": "wrong-but-long"})
...
async def test_deactivation_after_verify(fake_redis):
    app = await _app()
    sm = app.state.sessionmaker
    await seed_admin(sm, email="susp@platform.local", password=PW, active=False)
    async with asgi_client(app) as c:
        # Correct password against a deactivated admin → USER_DEACTIVATED (only after verify).
        good = await c.post("/api/auth/login", json={"email": "susp@platform.local", "password": PW})
        # Wrong password against the same deactivated admin → indistinguishable INVALID_CREDENTIALS.
        bad = await c.post("/api/auth/login", json={"email": "susp@platform.local", "password": "wrong-but-long"})
...
async def test_local_auth_disabled_short_circuits(fake_redis, monkeypatch):
    ...
    sm = app.state.sessionmaker
    await seed_admin(sm, email="x@platform.local", password=PW)
    ...
    async with asgi_client(app) as c:
        r = await c.post("/api/auth/login", json={"email": "x@platform.local", "password": PW})
...
async def test_db_error_fails_closed_not_401(monkeypatch):
    """A DB error during the admin lookup propagates (→ 500), never remapped to 401."""
    ...
    with pytest.raises(OperationalError):
        await service.login_local(boom_sessionmaker, settings, "a@platform.local", PW, ip="1.2.3.4")
...
async def test_lockout_trips_429_with_retry_after(fake_redis):
    ...
    sm = app.state.sessionmaker
    await seed_admin(sm, email="lock@platform.local", password=PW)
    async with asgi_client(app) as c:
        for _ in range(3):
            r = await c.post("/api/auth/login", json={"email": "lock@platform.local", "password": "wrong-but-long"})
            assert r.status_code == 401
        locked = await c.post("/api/auth/login", json={"email": "lock@platform.local", "password": PW})
...
async def test_over_length_password_rejected_pre_hash(fake_redis, monkeypatch):
    ...
    async with asgi_client(app) as c:
        r = await c.post("/api/auth/login", json={"email": "a@platform.local", "password": "p" * 4097})
